...
let docOne = addUID({name: 'Joey', age: 46 });
console.log(docOne);
...
let docTwo = addUID2({name: 'Joshua'});
console.log(docTwo);
...
const transError: BaseError<SubmitTransactionResponse> = {
    ...
    errors: { status: 'Declined', receiptNumber: 56689, error:{ message: 'Failed to submit transaction - Network is down', errorType: 'Network'}},
